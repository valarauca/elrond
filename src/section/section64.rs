...
use super::super::nom::{le_u32,le_u64,be_u32,be_u64,IResult};
use super::super::{
...
pub struct Section64<'a> {
    ...
    sections: Arc<[PreSection64]>
}
impl<'a> Section64<'a> {

    ///Build a new section
    fn new<E: ElfHeader<'a>+'a+?Sized>(data: &Arc<[PreSection64]>, index: usize, header: &E)
        -> Result<Section64<'a>,Fault>
    {
...
fn get_name<'a, E: ElfHeader<'a>+'a+?Sized>(data: &Arc<[PreSection64]>, header: &E, name_index: usize, index: usize)
...
impl PreSection64 {
    ...
    #[inline(always)]
    fn build<'a, E: ElfHeader<'a>+?Sized+'a>(temp: &[u8], header: &E)
        -> Result<PreSection64,Fault> 
    {
        //declare internal parsers
        named!(parse_pre_sect_64_le<PreSection64>,do_parse!(
            sh_name: le_u32 >>
        ...
        named!(parse_pre_sect_64_be<PreSection64>,do_parse!(
            sh_name: be_u32 >>
        ...
        match sect {
            ...
            IResult::Done(_,sh) => Ok(sh)
        }
    }
}
pub fn parse_section64<'a, E: ElfHeader<'a>+'a+?Sized>(header: &E)
    -> Result<Arc<[Arc<Section<'a>+'a>]>,Fault>
{
    ...
    if size == 0 || num == 0{
        return Ok(Arc::from(Vec::with_capacity(0)));
    }
...
impl<'a> Section<'a> for Section64<'a> {
    #[inline(always)]
